    ...
    <AdminShell>
      <div className="grid gap-8">
        ...
        <div className="border border-[#d9d9d9] bg-white">
          {messages.length === 0 ? (
            <div className="p-8 font-bold text-[#777777]">
              ...
              {messages.map((message) => (
                <article key={message.id} className="grid gap-3 p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <p className="text-xs font-black uppercase text-[#777777]">{message.status}</p>
                  </div>
                  <p className="text-sm font-bold text-[#777777]">{message.email} / {message.company ?? "No company"}</p>
                  <p className="font-black uppercase">{message.projectNeed}</p>
                  <p className="leading-7 text-[#444444]">{message.message}</p>
                </article>
              ))}
            </div>
          )}
